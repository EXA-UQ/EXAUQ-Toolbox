...
class MogpEmulator(AbstractGaussianProcess):
    ...
    **kwargs : dict, optional
    ...
    def update(
        ...
        """Update the current fitted MogpEmulator to new conditions.
        ...
        if new_design_pts is not None:
            if not isinstance(new_design_pts, Collection):
                raise TypeError(
                    f"Expected 'new_design_pts' to be of type collection of {Input.__name__}, but "
                    f"received {type(new_design_pts)} instead."
                )

            if any(not isinstance(x, Input) for x in new_design_pts):
                raise TypeError(
        ...
        super().update(
            new_design_pts, new_outputs, hyperparameters, hyperparameter_bounds
        )
